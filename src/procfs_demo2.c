/*
 * procfs_demo2.c --  another example of profs API.
 *                  use the file_operation way to manage the file.
 * use:
 * 	$sudo sh -c "echo hello world2 > /proc/buffer2k"
 * 	$cat /proc/buffer2k
 *	hello world2
 *
...
static int __init procfs_demo2_init(void)
{
...
static void __exit procfs_demo2_exit(void)
{
	remove_proc_entry(PROC_ENTRY_FILENAME, NULL);
}

module_init(procfs_demo2_init);
module_exit(procfs_demo2_exit);
MODULE_AUTHOR("HeZhiping");
MODULE_LICENSE("Dual BSD/GPL");
MODULE_DESCRIPTION("An another example to show how to use procfs APIs");
