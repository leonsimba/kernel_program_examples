...
#include <linux/seq_file.h>	/* for seq file */
...
static void scull_create_proc(void)
{
	...
	/* create /proc/scullmem and set read() is scull_read_procmem */
	create_proc_read_entry("scullmem", 0, NULL, scull_read_procmem, NULL);

	/* create /proc/scullseq */
...
int scull_open(struct inode *inode, struct file *filp)
{
	...
	/* for char dev, inode->i_cdev point to the struct cdev */
	...
	if ( (filp->f_flags & O_ACCMODE) == O_WRONLY) {
		...
		if (down_interruptible(&dev->sem))
			return -ERESTARTSYS;
		/* Trim the length of the device to 0 */
		scull_trim(dev);
		up(&dev->sem);
	}
	return 0;
                ...
                loff_t *f_pos)
{
	...
	item = (long)*f_pos / itemsize;    /* which qset */
	rest = (long)*f_pos % itemsize;
	s_pos = rest / quantum;    	   /* which quantum */
	 q_pos = rest % quantum;
